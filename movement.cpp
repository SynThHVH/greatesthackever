...
void Movement::AutoPeek() {
	...
	if (g_input.GetKeyState(g_menu.main.movement.autopeek.get()) && g_aimbot.m_stop) {
		Movement::QuickStop();
	}
}
...
void Movement::LimitSpeed(float max) {
	if (max <= 0.f) return;
	...
	float ratio = fminf(max / speed, speed);

	g_cl.m_cmd->m_forward_move *= ratio;
	g_cl.m_cmd->m_side_move *= ratio;
}
