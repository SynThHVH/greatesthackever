...
void Visuals::DrawSkeleton(Player* player, int opacity) {
	...
	for (int i{ }; i < hdr->m_num_bones; ++i) {
		...
		// world to screen both the bone parent bone then draw.
		if (render::WorldToScreen(bone_pos, bone_pos_screen) && render::WorldToScreen(parent_pos, parent_pos_screen)) render::line(bone_pos_screen.x, bone_pos_screen.y, parent_pos_screen.x, parent_pos_screen.y, clr);
	}
}
...
void Visuals::RenderGlow() {
	...
	if (!g_cl.m_local) return;
	if (!g_csgo.m_glow->m_object_definitions.Count()) return;
	...
	for (int i{ }; i < g_csgo.m_glow->m_object_definitions.Count(); ++i) {
		...
		// skip non-players.
		if (!obj->m_entity || !obj->m_entity->IsPlayer()) continue;

		// get player ptr.
		player = obj->m_entity->as< Player* >();
		if (player->m_bIsLocalPlayer()) continue;
		...
		if (enemy && !g_menu.main.players.glow.get(0)) continue;
		if (!enemy && !g_menu.main.players.glow.get(1)) continue;

		// enemy color
		if (enemy) color = g_menu.main.players.glow_enemy.get();

		// friendly color
		else color = g_menu.main.players.glow_friendly.get();
...
void Visuals::DrawBeams() {
	...
	if (!g_cl.m_local) return;
	if (!g_menu.main.visuals.impact_beams.get()) return;
	...
	if (!g_cl.m_processing) {
		...
		for (size_t i{ impact_count }; i-- > 0; ) {
			...
			if (impact->m_ignore) continue;

			// is this the final impact?
			// last impact in the vector, it's the final impact.
			if (i == (impact_count - 1)) is_final_impact = true;

			// the current impact's tickbase is different than the next, it's the final impact.
			else if ((i + 1) < impact_count && impact->m_tickbase != vis_impacts->operator[ ](i + 1).m_tickbase) is_final_impact = true;

			// this isn't our final impact
			else is_final_impact = false;
...
void Visuals::DebugAimbotPoints(Player* player) {
	...
	if (!data || data->m_records.empty()) return;

	LagRecord* front = data->m_records.front().get();
	if (!front || front->dormant()) return;

	// get bone matrix.
	BoneArray matrix[128];
	if (!g_bones.setup(player, matrix, front)) return;

	data->SetupHitboxes(front, false);
	if (data->m_hitboxes.empty()) return;

	for (const auto& it : data->m_hitboxes) {
		std::vector< vec3_t > p1{ };
		if (!player->SetupHitboxPoints(matrix, it.m_index, p1)) continue;

		for (auto& p : p1) p2.push_back(p);
	}
	...
	for (auto& p : p2) {
		...
		if (render::WorldToScreen(p, screen)) render::rect_filled(screen.x, screen.y, 2, 2, { 0, 255, 255, 255 });
	}
}