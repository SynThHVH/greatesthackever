...
float penetration::scale( Player* player, float damage, float armor_ratio, int hitgroup ) {
	...
	switch( hitgroup ) {
	case HITGROUP_HEAD:
		if ( has_heavy_armor ) damage *= 2.0f;
		else damage *= 4.f;
		break;
...
bool penetration::TraceToExit( const vec3_t &start, const vec3_t& dir, vec3_t& out, CGameTrace* enter_trace, CGameTrace* exit_trace ) {
	...
	while( dist <= 90.f ) {
		...
		// if this convar is called use the games way of clipping traces to all players
        if( g_csgo.sv_clip_penetration_traces_to_players->GetInt( ) ) game::UTIL_ClipTraceToPlayers( out, new_end, MASK_SHOT, nullptr, exit_trace, -60.f );
		
		// instead if that isn't being called in the server etc, it would be faster if we used our own function and clipped our trace to the specific entity that we are tracing at that time.
		else ClipTraceToPlayer(out, new_end, MASK_SHOT, exit_trace, (Player*)exit_trace->m_entity, -60.f);
...
bool penetration::run( PenetrationInput_t* in, PenetrationOutput_t* out ) {
    ...
    while( damage > 0.f /* && (damage * 4) >= in->m_damage_pen && (damage * 4) >= in->m_damage */) {
