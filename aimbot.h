...
struct AnimationBackup_t {
	...
	float            m_duck, m_body, m_move_weight, m_primary_cycle;
...
public:
	...
	bool       m_stop;
	float      m_hitbox, m_hitchance_amount, m_accuracy_amount;
...
public:
	...
	void apply();
	void NoSpread();
	bool CanHit(vec3_t start, vec3_t end, LagRecord* record, int box, bool in_shot = false, BoneArray* bones = nullptr);
