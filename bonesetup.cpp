...
bool Bones::BuildBones( Player* target, int mask, BoneArray* out, LagRecord* record ) {
	...
	*reinterpret_cast< int* >( uintptr_t( target ) + 0x260 ) = NULL;
	...
	math::AngleMatrix( record->m_abs_ang, record->m_extrapolated_origin, transform );

	// set non interpolated data.
	target->AddEffect( EF_NOINTERP );
	target->SetAbsOrigin( record->m_extrapolated_origin);
	...
	target->StandardBlendingRules( hdr, pos, q, record->m_lagcorrected_time, mask );
