...
void Aimbot::knife( ) {
	...
	for( const auto& t : m_targets ) {
		...
		const float max_range = g_cl.m_weapon_info ? g_cl.m_weapon_info->m_range : 3000;
		const auto distance = best->m_extrapolated_origin.dist_to(g_cl.m_local->m_vecOrigin());
		...
		for (const auto& a : m_knife_ang) {
			...
			if (!CanKnife(best, a, target.stab)) continue;
	...
	if( target.record ) {
		// set target tick.
		g_cl.m_cmd->m_tick = game::TIME_TO_TICKS( target.record->m_lagcorrected_time + g_cl.m_lerp );
		...
		// choke.
		if (g_csgo.m_cl->m_choked_commands < 13) *g_cl.m_packet = false;
	}
}
