...
void Client::DrawHUD() {
	if (!g_menu.main.misc.watermark.get()) return;
	...
	std::string text = tfm::format(XOR("rufarohook [dev] | delay: %ims | tick: %i | %s"), ms, rate, time.str().data());
...
void Client::ClanTag() {
	...
	auto SetClanTag = [&](std::string tag) -> void {
		...
		SetClanTagFn(tag.c_str(), XOR("nigger no name tapper hack"));
	};

	std::string szClanTag = XOR("synthco.de");
...
void Client::KillFeed() {
	if (!g_menu.main.misc.killfeed.get()) return;
	if (!g_csgo.m_engine->IsInGame()) return;
	...
	for (int i{ }; i < size; ++i) {
		...
		if (notice->fade == 1.5f) notice->fade = FLT_MAX;
	}
}
...
void Client::BackupPlayers(bool restore) {
	if (restore) {
		...
		for (int i{ 1 }; i <= g_csgo.m_globals->m_max_clients; ++i) {
			...
			if (!g_aimbot.IsValidTarget(player)) continue;
...
void Client::DoMove() {
	...
	if (m_weapon) {
		...
		if (m_weapon_type != WEAPONTYPE_GRENADE) m_weapon->UpdateAccuracyPenalty();
	...
	if (g_input.GetKeyState(g_menu.main.misc.last_tick_defuse.get()) && g_visuals.m_c4_planted) {
		...
		if (dt <= game::TICKS_TO_TIME(2)) m_cmd->m_buttons |= IN_USE;
	}
...
void Client::OnTick(CUserCmd* cmd) {
	...
	g_inputpred.PrePrediction();
}

void Client::SetAngles() {
	if (!g_cl.m_local || !g_cl.m_processing) return;
	...
	// set radar angles.
	if (g_csgo.m_input->CAM_IsThirdPerson()) g_csgo.m_prediction->SetLocalViewAngles(m_radar);
}
...
void Client::UpdateInformation() {
	...
	// fix landing anim.
	if (state->m_landing) m_angle.x = -12.f;
	...
	if (state->m_last_update_frame == g_csgo.m_globals->m_frame) state->m_last_update_frame -= 1;

	// call original, bypass hook.
	if (g_hooks.m_UpdateClientSideAnimation) g_hooks.m_UpdateClientSideAnimation(g_cl.m_local); //not real fix but why not

	// get last networked poses.
	g_cl.m_local->GetPoseParameters(g_cl.m_poses);

	// store updated abs yaw.
	g_cl.m_abs_yaw = state->m_move_yaw_ideal;
	...
	else if (state->m_velocity_length_xy > 0.1f) {
		if (state->m_on_ground) m_body = m_angle.y;
...
bool Client::CanFireWeapon() {
	...
	if (!m_player_fire) return false;
	if (m_weapon_type == WEAPONTYPE_GRENADE) return false;

	// if we have no bullets, we cant shoot.
	if (m_weapon_type != WEAPONTYPE_KNIFE && m_weapon->m_iClip1() < 1) return false;
	...
	if ((m_weapon_id == GLOCK || m_weapon_id == FAMAS) && m_weapon->m_iBurstShotsRemaining() > 0) {
		...
		if (g_csgo.m_globals->m_curtime >= m_weapon->m_fNextBurstShot()) return true;
	}
	...
	if (m_weapon_id == REVOLVER) {
		...
		// mouse1.
		if (!m_revolver_fire) {
			if ((act == 185 || act == 193) && m_revolver_cock == 0) return g_csgo.m_globals->m_curtime >= m_weapon->m_flNextPrimaryAttack();
			return false;
		}
	}

	// yeez we have a normal gun.
	if (g_csgo.m_globals->m_curtime >= m_weapon->m_flNextPrimaryAttack()) return true;
...
void Client::UpdateRevolverCock() {
	...
	// reset properly.
	if (m_revolver_cock == -1) m_revolver_cock = 0;
	...
	if (m_revolver_query == m_revolver_cock) {
		...
		// apply inattack.
		if (g_menu.main.config.mode.get() == 0 && m_revolver_query > m_revolver_cock) m_cmd->m_buttons |= IN_ATTACK;

		// count cock ticks. ( fucking faggots counting cocks )
		// do this so we can also count 'legit' ticks
		// that didnt originate from the hack.
		if (m_cmd->m_buttons & IN_ATTACK) m_revolver_cock++;
	...
	if (m_revolver_cock > 0) m_cmd->m_buttons &= ~IN_ATTACK2;
}

void Client::UpdateIncomingSequences() {
	if (!g_csgo.m_net) return;
	...
	while (m_sequences.size() > 2048) m_sequences.pop_back();
}